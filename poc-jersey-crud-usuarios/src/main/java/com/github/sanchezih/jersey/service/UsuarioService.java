...
public class UsuarioService {
	...
	public Response updateUser(Usuario usuario) {
		//recibe datos del usuario a modificar, busca por name y reemplaza el valor actual con el username que viene en el body
		Usuario found = null;
		for (int i = 0; i < listaUsuarios.size(); i++) {
			if (listaUsuarios.get(i).getName().equalsIgnoreCase(usuario.getName())) {
				found = listaUsuarios.get(i);
			}
		}
